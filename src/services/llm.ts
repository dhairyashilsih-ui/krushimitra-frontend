...
import { queryCloudLLMStream, queryCloudLLM, getCloudLLMConfig, type CloudLLMConfig } from './cloudLLM';
...
/**
 * Summarizes the active chat history + the old long-term memory into a new persistent string
 * This runs in the background when the 10-message quota is hit.
 */
export async function summarizeConversation(
  ...
  const mode = getLLMMode();
  if (mode === 'local') {
    // Basic fallback for local context limit
    console.log("Local mode skipping deep summarization due to capacity limits.");
    return currentMemory;
  }

  try {
    const config = getCloudLLMConfig();
...
Keep it strictly under 50 words. Be concise. Reply ONLY with the updated memory profile text.`
    ...
    console.log('🧠 Triggering Background Summarization...');
    const result = await queryCloudLLM(messages, config);
    console.log('🧠 New Memory Profile Generated:', result);
    return result;

  } catch (error) {
    console.error('Memory summarization failed:', error);
    return currentMemory; // Fallback to existing memory on failure
  }
}
